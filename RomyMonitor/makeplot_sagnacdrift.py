...
try:
    arr_mltiU, mltiU_cumsum, mlti_cumsum_percent, mltiU_inter_sec = __get_mlti_statistics(mltiU_t1, np.array(beatU.times_utc), plot=True, ylog=True)
except Exception as e:
    print(e)
    print(f" -> failed to get MLTI statistics for RU")

try:
    arr_mltiV, mltiV_cumsum, mlti_cumsum_percent, mltiV_inter_sec = __get_mlti_statistics(mltiV_t1, np.array(beatV.times_utc), plot=True, ylog=True)
except Exception as e:
    print(e)
    print(f" -> failed to get MLTI statistics for RV")

try:
    arr_mltiZ, mltiZ_cumsum, mlti_cumsum_percent, mltiZ_inter_sec = __get_mlti_statistics(mltiZ_t1, np.array(beatZ.times_utc), plot=True, ylog=True)
except Exception as e:
    print(e)
