...
config['component'] = "Z" ##  O=outside | I=infrasound | F=filtered
...
config['seed'] = f"XX.RY01..HH{config['component']}"

config['ring'] = config['seed'].split(".")[1]


# config['path_to_data'] = f"/export/data/LNM/data/GRF/{config['array']}/"
config['path_to_data'] = f"/import/freenas-ffb-01-data/ModalAnalysis/data/SDS/"
...
config['outname'] = f"{config['year']}_{sta}_{cha[2]}_{config['interval']}"

config['outpath'] = f"/import/freenas-ffb-01-data/ModalAnalysis/data/PSDS/"

if not os.path.exists(config['outpath']+f"{sta}"):
    os.mkdir(config['outpath']+f"{sta}")

if not os.path.exists(config['outpath']+f"{sta}"+f"/{cha}"):
    os.mkdir(config['outpath']+f"{sta}"+f"/{cha}")

config['outpath'] = f"/import/freenas-ffb-01-data/ModalAnalysis/data/PSDS/{sta}/{cha}/"
