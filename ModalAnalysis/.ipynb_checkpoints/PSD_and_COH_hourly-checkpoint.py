...
if len(sys.argv) > 1:
    ...
    config['seed1'] = "GR.FUR..HHZ"  ## F = infrasound | O = absolute
    config['seed2'] = "XX.RY01..HHZ"


net1, sta1, loc1, cha1 = config['seed1'].split('.')
net2, sta2, loc2, cha2 = config['seed2'].split('.')
...
## specify time series interval and overlap
...
## specify output path for data
config['outpath1'] = archive_path+f"ModalAnalysis/data/PSDS/{config['sta1']}/"
config['outpath2'] = archive_path+f"ModalAnalysis/data/PSDS/{config['sta2']}/"
config['outpath3'] = archive_path+f"ModalAnalysis/data/PSDS/{config['sta2']}_coherence/"
