...
#     fbands = [(fc - ddf, fc + ddf) for fc in fcenter]
    ...
    for ii, comp in enumerate(["Z", "N", "E"]):
        ...
        if plot:
            ...
            ax7.scatter(fcenter, cc2, s=60, marker="s", label="BSPF - RPFO_inner", zorder=2, color="w", edgecolors=colors[comp])
            ax7.plot(fcenter, cc2, ls="-", color=colors[comp])

            ax7.scatter(fcenter, cc3, s=30, label="BSPF - PRFO_mid", zorder=2, color="w", edgecolors=colors[comp])
