...
class I95SDSClient(object):
    ...
    def plot_all_data(self, starttime, endtime, type='image', nslc=None,
        ...
        if type == 'image':
            ...
            vmin_global = np.nanmin(data['i95'].filled(np.nan))
            ...
            vmax_global = np.nanpercentile(
                data['i95'].filled(np.nan), q=self.vmax_clip_percentile)
            vmin = None
            vmax = None
            if global_norm:
                vmin = vmin_global
    ...
    def _plot_image(self, ax, data, label, cmap=None, colorbar=True,
        ...
        if vmax is None:
            ...
            vmax = np.nanpercentile(
                data['i95'].filled(np.nan) * scaling_factor,
                q=self.vmax_clip_percentile)
            # print vmax
        # print(vmin, vmax)
        ...
        if data.ndim == 1 or global_norm:
            ...
            for i, data_ in enumerate(data[::-1]):
                ...
                #                         q=self.vmax_clip_percentile)
                vmax = np.nanpercentile(
                    data_['i95'].filled(np.nan) * scaling_factor,
                    q=self.vmax_clip_percentile)
    ...
    def _plot_violin(self, ax, data, labels, verbose=False, percentiles=None,
        ...
        if verbose:
            ...
            for d, label in zip(data, labels):
                ...
                if not d.size or np.all(np.isnan(d.filled(np.nan))):
                    ...
                    values = np.nanpercentile(d.filled(np.nan), q=percentiles)
        ...
        for i, d in enumerate(data):
            if not d.size:
                continue
            # work around numpy/numpy#21524
            # value = np.nanpercentile(d, 99)
            value = np.nanpercentile(d.filled(np.nan), 99)
            y_max = max(y_max, value)
            # work around numpy/numpy#21524
            # value = np.nanpercentile(d, 95) * 1.3
            value = np.nanpercentile(d.filled(np.nan), 95) * 1.3
            y_max = min(y_max, value)
            y_min = min(y_min, np.nanmin(d.filled(np.nan)))
...
if __name__ == '__main__':
    ...
    # i95_client = I95SDSClient('/bay200/mseed_online/archive/')

    # example that merges data from HHZ and EHZ channels
    start = UTCDateTime(2024, 1, 1)
    end = UTCDateTime(2024, 3, 31)

    # fig, ax = i95_client.plot('BW', 'ROMY', '10', 'BJZ', start, end, show=False)
    # plt.show()

    fig, ax = i95_client.plot('BW', 'GRMB', '', 'HHZ', start, end, show=False)
    plt.show()

    fig, ax = i95_client.plot('BW', 'BIB', '', 'HHZ', start, end, show=False)
    plt.show()

    fig, ax = i95_client.plot('BW', 'TON', '', 'HHZ', start, end, show=False)
    plt.show()

    fig, ax = i95_client.plot('BW', 'ALFT', '', 'HHZ', start, end, show=False)
    plt.show()
    ...
    data, labels = i95_client.get_availability(
        starttime=UTCDateTime(2024, 3, 1), endtime=UTCDateTime(2024, 3, 31),
        fast=True)
