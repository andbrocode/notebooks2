...
def __compute_backazimuth_noise(confx, rot0, acc0, cat_event, fmin, fmax, cc_thres=None, plot=False):
    ...
    confx['tbeg'] = rot[0].stats.starttime
    confx['tend'] = rot[0].stats.endtime

    ## Eventtime
    if cat_event is not None:
        confx['eventtime'] = UTCDateTime(cat_event.origins[0].time)
    else:
        confx['eventtime'] = UTCDateTime(rot[0].stats.starttime)
...
#     config['step'] = 1
    ...
    out3 = __compute_backazimuth_tangent(
                                        rot,
                                        acc,
                                        win_time_s= confx['win_length_sec'],
                                        overlap=confx['overlap']/100,
    ...
    if plot:
        try:
            ...
            for _ax in [ax0, ax1, ax2]:
                ...
                _ax.set_xlim(0, (confx['tend']-confx['tbeg'])*1.15)
            ...
            ax0.set_title(f" {confx['tbeg'].date}  {str(confx['tbeg'].time).split('.')[0]}-{str(confx['tend'].time).split('.')[0]} UTC | f = {round(fmin ,3)}-{round(fmax,3)} Hz | T = {confx['win_length_sec']} s | {confx['overlap']} % overlap")

            ax5.set_xlabel("Time (s)")
            try:
                fig3.savefig(confx['path_to_figs']+f"BAZ_{confx['tbeg']}.png", format="png", dpi=150, bbox_inches='tight')
            except Exception as e:
                print(e)
...
## ---------------------------------------
    ...
    try:
        out = __compute_backazimuth_noise(conf, rot, acc, None, config['fmin'], config['fmax'], cc_thres=config['cc_threshold'], plot=True);
