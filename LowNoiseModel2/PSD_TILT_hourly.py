...
config['opath'] = data_path+f"LNM2/PSDS/{config['array']}/"

config['oname'] = f"{config['year']}_{config['station']}-{config['component']}_{config['interval']}"
...
def main(config):
    ...
    for date in date_range(str(config['date1'].date), str(config['date2'].date), days):
        ...
        ## load data to stream
        st = __read_sds(config['path_to_sds'], config['seed'], config['tbeg'], config['tend'])
        ...
        try:
            ...
            ## renove sensitivity
            st = __conversion_to_tilt(st, confTilt[config['array']])
