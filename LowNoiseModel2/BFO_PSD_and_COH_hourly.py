...
if len(sys.argv) > 1:
    ...
    config['seed1'] = "II.BFO.10.LDO"  ## F = infrasound | O = absolute
    config['seed2'] = "GR.BFO..LHZ"
...
def main(config):
    ...
    for date in date_range(str(config['date1'].date), str(config['date2'].date), days):
        ...
        try:
            st2, inv2 = __querrySeismoData(
                                        seed_id=config['seed2'],
                                        starttime=config['tbeg']-offset_sec,
                                        endtime=config['tend']+offset_sec,
                                        repository="online",
                                        path=None,
...
#             continue
        ...
        ## conversion
        if "O" in st1[0].stats.channel and "FFBI" in st1[0].stats.station:
        ...
        if "O" in st1[0].stats.channel and "BFO" in st1[0].stats.station:

            if config['unit'] == "Pa":
                for tr in st1:
                    tr.data = tr.data *0.1   # gain=1 sensitivity_reftek=6.28099e5count/V; sensitivity_mb2005 = 1 mV/hPa
            elif config['unit'] == "hPa":
                for tr in st1:
                    tr.data = tr.data *0.001   # gain=1 sensitivity_reftek=6.28099e5count/V; sensitivity_mb2005 = 1 mV/hPa
...
#                tr.data = tr.data *1.589e-6 /0.02  # gain=1 sensitivity_reftek=6.28099e5count/V; sensitivity_mb2005=0.02 V/Pa
        ...
        try:
            ...
            if "BW.DROMY" in config['seed2']:
                ...
                st1 = st1.filter("bandpass", freqmin=1e-4, freqmax=0.1, corners=4, zerophase=True)
                st2 = st2.filter("bandpass", freqmin=1e-4, freqmax=0.1, corners=4, zerophase=True)
